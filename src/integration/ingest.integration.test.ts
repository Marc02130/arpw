...
import { NFR7_BIBLIOGRAPHY, NFR7_PROBE, fixturePdfBytes } from '../lib/nfr7Fixture'
import {
  assertSupabaseUp,
  createConfirmedUser,
  deleteUser,
    ...
    async (ctx) => {
      ...
      try {
        ...
        const { data: vectors, error: vecError } = await user.client
          ...
          .select('chunk_text, embedding_model, section, page')
          .eq('file_id', fileId)
        ...
        expect(vectors?.some((row) => row.section === 'Methods' && (row.chunk_text ?? '').includes(NFR7_PROBE))).toBe(
          true
        )
        expect(
          vectors
            ?.filter((row) => row.section === 'Methods')
            .every((row) => !(row.chunk_text ?? '').includes(NFR7_BIBLIOGRAPHY))
        ).toBe(true)
        expect(vectors?.some((row) => row.section === 'References')).toBe(true)
        expect(vectors?.every((row) => row.page === 1 || row.page === null)).toBe(true)
